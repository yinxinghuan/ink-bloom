...
Ink Bloom 是一个 `social` 类型的移动端小游戏：AlterU 感官 · 墨流大理石纹。点一下墨环绽放，拖动把嵌套色环梳成羽状大理石纹，松手晾到公共晾纸绳上。看到别人的纹纸盖一枚蜡封印 → 作者收到通知。搅墨即治愈，盖印即社交。

## 2. Visual Design

- 整体布局：页面占用 100vw x 100vh，主体验居中，HUD 与操作区覆盖在游戏层上方，移动端以单手操作为优先。
- 背景与配色：主要颜色使用 #fff、#11131c、rgba(255, 255, 255, 0.92)、rgba(0, 0, 0, 0.4)、rgba(255, 255, 255, 0.4)、rgba(0, 0, 0, 0.5)、rgba(0, 0, 0, 0.55)、rgba(20, 22, 34, 0.5)；高亮元素用于可点击目标、得分、结果或稀有状态。
